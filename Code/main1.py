...
main = Tk()
main.title("Authentication of product & counterfeit elimination using blockchain baased QR code")
    ...
    for i in range(len(blockchain.chain)):
        if i > 0:
            ...
            if arr[5] == digital_signature:
                ...
                text.insert(END,"certificate id                   : "+arr[0]+"\n")
                text.insert(END,"Student Name                 : "+arr[1]+"\n")
                text.insert(END,"University         : "+arr[2]+"\n")
                text.insert(END,"Address Details              : "+arr[3]+"\n")
                text.insert(END,"Scan Date & Time             : "+arr[4]+"\n")
                text.insert(END,"Certificate Barcode Digital Sign : "+arr[5]+"\n")
                output='<html><body><table border=1>'
                output+='<tr><th>Block No</th><th></th>certificate id<th>Product Name</th><th>Company/User Details</th><th>Address Details</th><th>Scan Date & Time</th>'
    ...
    if len(pid) > 0:
        for i in range(len(blockchain.chain)):
            if i > 0:
                ...
                if arr[0] == pid:
                    output = ''
                    text.insert(END,"Certificate Details extracted from Blockchain using Product ID : "+pid+"\n\n")
                    text.insert(END,"Hall Ticket Id:                   : "+arr[0]+"\n")
                    text.insert(END,"Full Name                 : "+arr[1]+"\n")
                    text.insert(END,"CGPA         : "+arr[2]+"\n")
                    text.insert(END,"Address              : "+arr[3]+"\n")
                    text.insert(END,"Scan Date & Time             : "+arr[4]+"\n")
                    text.insert(END,"Certificate Barcode Digital Sign : "+arr[5]+"\n")
                    output='<html><body><table border=1>'
                    output+='<tr><th>Block No</th><th>Hall Ticket Id</th><th>Full Name</th><th>CGPA</th><th>Address</th><th>Scan Date & Time</th>'
                    output+='<th>Certificate Barcode Digital Signature</th></tr>'
                    output+='<tr><td>'+str(i)+'</td><td>'+arr[0]+'</td><td>'+arr[1]+'</td><td>'+arr[2]+'</td><td>'+arr[3]+'</td><td>'+arr[4]+'</td><td>'+arr[5]+'</td></tr>'
                    f = open("output.html", "w")
                    f.write(output)
                    f.close()
...
font = ('times', 15, 'bold')
title = Label(main, text='Authentication of product & counterfeit elimination using blockchain baased QR code')
...
l1 = Label(main, text='Certificate Id :')
...
l3 = Label(main, text='University Name :')
...
l4 = Label(main, text='University Address  :')
...
searchButton = Button(main, text="Retrieve Certificate Data", command=searchProduct)
